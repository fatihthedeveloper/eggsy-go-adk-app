...
func (n *NativeImplTransactionTrackerToolsBuilder) CreateTransactionTool() (tool.Tool, error) {
	fn := func(ctx tool.Context, args createTransactionArgs) (createTransactionResult, error) {
		ctxx := utils.NewContext(ctx)

		res, err := n.TrxService.Create(ctxx, eggsytransactionsdk.TransactionCreateReq{
			Email:           args.Email,
			Amount:          args.Amount,
	...
	newTool, err := functiontool.New(functiontool.Config{
		...
		Description: "Creates a transaction record",
	}, fn)
